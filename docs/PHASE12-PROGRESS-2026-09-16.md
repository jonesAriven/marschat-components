...
>
> ⏳ **时效声明（2026-09-19 追加）**：本文是 **2026-09-16 18:00 的进度快照**，**已被 `docs/PHASE12-SUMMARY-2026-09-17.md` 取代**，其待办结论**已吸收进 `docs/STATUS.md`**。
> ⚠️ 本文 §4 待办表中多项（#1 六应用升 0.8.8 / #2 BFF 移出 create/update / #3 跨应用回归 / #6 ADR 入库）**已于 09-17/09-18 完成**，**切勿照本文待办直接开工** —— 一律以 `STATUS.md` 为准。
...
11. **探针账号**：auth-center 超管 `admin`（**口令见 Vaultwarden，严禁写入任何文档**）；测试脚本口令运行时随机生成、不落盘。
