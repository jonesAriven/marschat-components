...
- **修改密码**：一律走中心——登录页「忘记密码」或中心管理台重置。
  - ✅ **2026-09-18 落地（R6 / P1-4）**：portal 与 activecode 的本地改密端点已**下线为 410 Gone**（此前会真的改本地影子口令而中心不变 → 身份分裂，浏览器实测复现）；portal 下拉项改为「重置密码（走统一认证）」直达中心找回页。
  - 🟢 **正确范式 = kb-web**：`PUT /kb/api/user/password` 经 **kb-gateway 路由代理到中心**，实测真的改中心口令。新应用照此接入；无代理能力的一律下线入口 + 引导。
  - ❌ **严禁**改回本地口令比对（等于给身份分裂留后门）。
...
> **浏览器 E2E 断言四条铁律（2026-09-17 用 12 次脚本级误判换来的）**：
> ① 点元素要**精确匹配 `<button>` 全等文本**并用**真实鼠标事件**派发到其 bbox 中心 —— 按文本长度排序取最短会取到**父节点**，点父节点无效（事件只向上冒泡）；
> ② 成功判据用「**落点 URL + 接口状态码 + 身份**」，**不要用「无 toast」**（成功 toast 也是 toast）；
...
| **34** | **凡「按用户名认身份」的逻辑，一律从 access_token 的 `username` 声明取，禁用 `sub`** —— 中心 SAS 的 `sub` 是**用户 ID**，且 **id_token 不签发 username**（只有 sub）；一端取 sub、一端取 username 必然失败 | activecode SSO 回调恒报「username 与 token 不一致」（缺陷 D1，2026-09-17 修复） |
...
| **F4** ✅ | 应用本地「修改密码」端点：portal / activecode 仍会**真的改本地影子口令**（中心不变 → 身份分裂） | **2026-09-18 已修（R6）**：端点下线为 **410 Gone** + 引导走中心；portal 下拉改「重置密码（走统一认证）」。参考实现 = kb-web（经 kb-gateway 代理到中心）。**不要**再改回本地比对 |
