...
>
> **版本基线（2026-09-15，Phase 10 收口后，全部为线上实测态）**
...
> 已接入 6 应用：portal / activecode / kb-web / cosmic / kb-ops / infra-monitor（**六个自研应用全部接入，初衷达成**）。
...
| 账密（独立登录） | 各应用本地（或 BFF） | 纯 SSO 应用（如 kb-ops）必须 `showLocalLogin:false`，否则渲染必 403 的死表单 |
...
| F2 🟡 | activecode 以 sub（用户ID）作映射键，账号映射实际按 ID 匹配 | 后端改读 `username` 声明 |
